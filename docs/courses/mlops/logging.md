...
keywords: logging, mlops, machine learning
...
> When it comes to saving our logs, we could simply [upload](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/s3-uploading-files.html){:target="_blank"} our logs to a cloud blog storage (ex. S3 or Google Cloud Storage). Or for a more production-grade logging option, we could consider the [Elastic stack](https://www.elastic.co/what-is/elk-stack){:target="_blank"}.

In the [next lesson](documentation.md){:target="_blank"}, we'll learn how to document our code and automatically generate high quality docs for our application.
